...
指标侧已经先行落地同一模式：数据集发布后的候选指标会由确定性规则固定可执行事实，再由 LLM 补充名称、说明、口径文字、周期说明、血缘摘要和标签；语义文档使用 `Qwen3-Embedding-4B` 生成 2560 维 `halfvec`，写入 `platform.metric_semantic_documents` 并通过 HNSW cosine 索引检索。该实现可复用其 Provider、Worker、租约重试、降级和租户隔离方式，但不能把指标文档表直接复用于表/字段资产。
