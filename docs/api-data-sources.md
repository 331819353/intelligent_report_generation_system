...
- `POST /api/v1/excel-files/{id}/versions`：覆盖当前源文件并创建不可变新版本；引用该文件的数据源回到待验证状态并推进版本，测试成功后重新启用。
...
1. 页面选择文件后以不含扩展名的文件名自动填写数据源名称，并以“规范化文件名 + 扩展名”生成稳定编码。新编码只校验文件扩展名、大小和 CSV 方言配置，将原始文件保存到 MinIO，创建不可变文件版本，再以响应中的 `id` 作为文件数据源的 `fileAssetId`。这一步不打开工作簿、不解析 Sheet，也不创建映射表；稳定编码已存在时改走版本上传接口，覆盖该数据源的当前文件指针，不重复创建数据源。
2. 在数据表资产页点击“新增数据表”后调用 `POST /api/v1/data-sources/{id}/file-inspection`。接口读取当前文件版本，对每个 Sheet 最多读取前 10 行，确定表头行、空行策略和字段类型并返回预览。确定性解析方案写入 `file_asset_inspections`，与当前文件版本一对一关联，不修改原始文件版本。用户可以选择首次或已经映射过的 Sheet；后台对每个 Sheet 重新读取真实表头及最多 3 行内容，将 Sheet 名称、字段类型、表头和样本值一起传给 LLM，用于生成表业务名称、字段业务名称及字段描述。

文件数据源的“数据表资产”页不提供数据库元数据的增量/全量刷新或表级结构刷新，统一显示“重新上传文件”。重新上传复用原文件资产并创建不可变新版本，随后重新验证数据源；用户再通过“新增数据表”预览、选择并重新映射 Sheet。数据库数据源仍保留原有元数据刷新能力。
