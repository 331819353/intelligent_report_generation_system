...
整表映射完成时会在同一租户事务内确保其默认单表数据集存在并执行受控系统发布，拓扑仅为“数据节点 → 结束节点”。ODS 是字段合同和精确来源版本的虚拟映射，不复制源明细、不登记数仓物化；文件或源表没有主键时 `outputGrain.keyFields` 保持为空，不会用首列猜测唯一性。保持该源行粒度的 DWD 会沿用这个无业务键合同，物化时只执行行数质量门，不会把普通标识字段升级为唯一键。ODS/DWD 设计预览按精确来源最多读取 100 行，DWD/DIM 正式构建才全量回源落仓。若三轮增量重试后仍有缺失，系统使用物理字段和已应用的 LLM 结果创建 `DRAFT / ODS` 降级草稿，不移动发布指针；用户可立即进入数据集中心修改，也可在失败明细或表资产操作中进入“手工完善”。系统只自动刷新从未被人工保存、回滚或提交审批的降级草稿，不覆盖用户修改；系统草稿被增量刷新后仍可依据精确草稿修订、哈希、零审批和零发布历史事实安全升级为首个发布版本。
...
“新增数据表”中显式选择已停用的既有表等同于重新纳管：服务端会把其 `managementStatus` 恢复为 `ENABLED`，重新执行本次 FULL 元数据映射，并在完成边界补建或恢复默认 ODS。表资产页直接点击“恢复”时，如果当前业务元数据已经完整，也会在同一事务内对账默认 ODS；这样无需再额外执行一次全量刷新，也不会出现刷新任务成功但数据集仍缺失的中间状态。
