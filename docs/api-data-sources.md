...
1. 新建数据源时只校验文件扩展名、大小和 CSV 方言配置，将原始文件保存到 MinIO，创建不可变文件版本，再以响应中的 `id` 作为文件数据源的 `fileAssetId`。这一步不打开工作簿、不解析 Sheet，也不创建映射表。
2. 在数据表资产页点击“新增数据表”后调用 `POST /api/v1/data-sources/{id}/file-inspection`。接口读取当前文件版本，对每个 Sheet 最多读取前 10 行，确定表头行、空行策略和字段类型并返回预览。确定性解析方案写入 `file_asset_inspections`，与当前文件版本一对一关联，不修改原始文件版本。用户选择 Sheet 并提交后，才进入后台 LLM 元数据完善和映射表创建流程。
