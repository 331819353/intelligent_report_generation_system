...
任务状态为 `QUEUED`、`RUNNING`、`SUCCEEDED`、`FAILED` 或 `CANCELLED`；`stage`
实时返回 `QUEUED`、`ADDRESS`、`PORT`、`DATABASE` 或 `AUTHENTICATION`。连接器只以
有界 NDJSON 控制事件向专用 worker 报告这些固定枚举，worker 在租约内写入当前阶段，
页面通过任务轮询把已完成阶段标为通过、当前阶段标为检测中，并在稳定错误码对应阶段
停止；阶段事件不包含地址、账号、密码或驱动原始错误。测试证据
...
Web 界面的审批入口只位于工作台“审批中心”卡片：有 `DATA_SOURCE:PUBLISH` 权限且不是申请人的用户点击卡片，在审批弹窗内通过或驳回。数据源配置页不展示审批按钮；申请人只在该页提交或撤销。审批通过即在同一事务中上线冻结版本，不再出现第二个“发布”动作。
...
整表映射完成时会在同一租户事务内先判断来源粒度，再确保默认单表数据集存在并执行受控系统发布，拓扑仅为“数据节点 → 结束节点”。事件、交易、快照等原始明细表保守归入 `ODS`；只有同时具备度量、明确粒度字段，并由技术表名或业务元数据明确证明已经在源端汇总的表，才归入 `DWS / sourceMode=PRE_AGGREGATED`。两者在发布事务中登记精确版本的全量入仓任务，分别写入 ODS 或 DWS 层；普通建模 DWS 仍严格要求 DWD/DIM 上游和聚合逻辑。文件或源表没有主键时 `outputGrain.keyFields` 保持为空，不会用首列猜测唯一性。保持源行粒度的 DWD 会沿用这个无业务键合同，物化时只执行行数质量门，不会把普通标识字段升级为唯一键。未发布映射数据集与 DWD 设计预览按精确来源最多读取 10 行，发布后的正式构建才全量回源落仓。若三轮增量重试后仍有缺失，系统使用物理字段和已应用的 LLM 结果创建未发布的映射降级草稿，不移动发布指针；用户可立即进入数据集中心修改，也可在失败明细或表资产操作中进入“手工完善”。系统只自动刷新从未被人工保存、回滚或提交审批的降级草稿，不覆盖用户修改；系统草稿被增量刷新后仍可依据精确草稿修订、哈希、零审批和零发布历史事实安全升级为首个发布版本。
...
任务状态可取 `QUEUED`、`RUNNING`、`SUCCEEDED`、`PARTIAL` 或 `FAILED`。查询响应返回 `total`、`completed`、`succeeded`、`skipped`、`failed`、`stage` 和 `currentTable`；存在失败项时还会返回可选的 `failures`，逐项包含库/Schema、表名、稳定错误码和安全文案。`logs` 是按时间排序的安全处理摘要，包含任务领取、表阶段、LLM 轮次、模型调用状态、实际模型名称和耗时，最多返回 300 条；它从任务状态与 AI 审计计量派生，不包含提示词、样本值、模型响应正文、上游错误正文、请求标识或输入哈希。`completed` 是成功、跳过和失败之和，页面进度条使用 `completed / total`，不按时间伪造进度。单表失败不会阻断后续表，响应和审计不包含样本数据、连接器原始错误或模型正文。worker 使用租户 RLS 事务、数据库租约和独立心跳领取任务；AI 成功与任务项及结构哈希绑定，租约恢复可直接收口已提交结果，API 或页面关闭不会中止任务。

数据源的修改、测试、暂停/恢复和删除操作管理连接本身；表资产的修改、字段映射、刷新、停用/恢复和删除操作管理 PostgreSQL 中的资产记录，两组生命周期相互独立。字段映射只允许修改业务名称、说明、标签、语义类型、敏感级别和人工锁定，不改写源库物理字段名或技术类型。`POST /api/v1/assets/tables/{id}/manual-completion` 使用 `expectedVersion + expectedStructureHash` 做乐观校验，要求表和全部活动字段的业务名称、说明、标签完整且字段语义类型已设置；成功后锁定人工结果、推进当前结构 marker，并按来源粒度创建或刷新对应 ODS/DWS 映射数据集。

“新增数据表”中显式选择已停用的既有表等同于重新纳管：服务端会把其 `managementStatus` 恢复为 `ENABLED`，重新执行本次 FULL 元数据映射，并在完成边界补建或恢复默认 ODS/DWS 映射。表资产页直接点击“恢复”时，如果当前业务元数据已经完整，也会在同一事务内对账默认映射数据集；这样无需再额外执行一次全量刷新，也不会出现刷新任务成功但数据集仍缺失的中间状态。
...
行数、列数、单元格、单行或整响应超限返回稳定的 `QUERY_*` 代码。DWD/DIM 正式回源使用
NDJSON 的 1 MiB/单元格、4 MiB/行、1 GiB/整流边界；Go 在 JSON 解码/NDJSON
消费前独立复核。`WAREHOUSE_STAGE_MAX_BYTES` 默认 1 GiB，按每租户、每物化任务
同时约束数据库回源流和 Excel/CSV 的逻辑 staging 行；两条路径任一超限都不能产生
可激活的部分结果。文件对象读取受租户 `max_excel_file_bytes` 限制；XLSX 采用
逐行解析和分批 COPY，展开总量受 8 倍压缩比与 2 GiB 绝对上限双重保护，
worksheet XML 超过 16 MiB 时使用临时文件。预算、取消或客户端断流会先关闭物理连接再关闭服务端游标，
未排空连接绝不回池；
...
- `POST /api/v1/excel-files/{id}/inspection`：解析当前不可变文件版本，返回 Sheet、表头、字段类型和有界预览，并固化确定性解析方案；供 AI 附件创建流程在数据源登记前使用。
...
上传受租户 `max_excel_file_bytes` 限制；没有租户级覆盖时默认 256 MiB。请求体超限返回
HTTP 413 和稳定错误码 `EXCEL_FILE_TOO_LARGE`，损坏的 multipart 表单才返回
`INVALID_EXCEL_UPLOAD`。原始文件在 API 到 MinIO 之间流式传输并同步计算 SHA-256，
不会再把整份大文件复制到应用内存。系统支持 `.xlsx`、`.xls` 和 `.csv`。ODS
...
2. 在数据表资产页点击“新增数据表”后调用 `POST /api/v1/data-sources/{id}/file-inspection`。接口读取当前文件版本，对每个 Sheet 最多返回前 10 行，确定表头行、空行策略和字段类型并返回给当前有权限的管理页面预览。XLSX 控制面读取只构造包含前 20 个物理行的有界预览包，既覆盖前十行表头探测和后十行数据样本，也不会为了预览而解压超大 worksheet 的剩余部分；全量查询和物化仍保留原有展开体积与行数硬限制。确定性解析方案写入 `file_asset_inspections`，与当前文件版本一对一关联，不修改原始文件版本。用户可以选择首次或已经映射过的 Sheet；后台始终使用 Sheet 名称、字段类型和真实表头，默认不把内容行发给 LLM。只有本次任务选择且租户策略允许 `MASK/RAW` 时才重新读取最多十行，并按上文规则脱敏或发送原值。Excel/CSV 表和字段的业务名称必须为中文：中文表头保持中文含义，英文表头由 LLM 翻译为中文；字段业务描述也必须包含中文。原始文件表头仍保存在技术字段名称中，用于文件读取、中文展示、血缘和追溯，不会被 LLM 覆盖。

数据源 AI 助手提供附件快捷流程：用户可在对话输入区直接选择 Excel/CSV，页面先上传文件资产，再调用资产级 `inspection` 接口；解析成功后按文件名和校验和生成不冲突的名称与编码，创建或更新 Excel 数据源并立即提交文件可用性测试。对话只展示 Sheet、字段名和推断类型摘要，原始单元格与预览行不会进入配置模型；用户确认后仍需点击“发布”进入原有审批流程。
