...
直接调用本接口时，服务只向模型发送表字段技术元数据和现有业务元数据，不发送连接串、密钥或数据样本；经用户授权的数据源导入与刷新流程可额外发送每表最多三行样本。`metadata-completion-v2` 会按本次输入动态固定表 ID、字段 ID 集合和字段数量；模型标签先按首次出现顺序去重，再执行本地枚举校验。未知、重复或缺失的资产 ID，以及非法标签、语义类型、敏感级别、置信度和不安全文本都会使整次任务失败，不写入建议或正式资产。
