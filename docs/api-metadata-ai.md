...
直接调用本接口时，服务只向模型发送表字段技术元数据和现有业务元数据，不发送连接串或密钥。Web 产品中的数据源导入与刷新任务默认选择 `MASK`，读取最多十行业务样本并在应用进程内完成格式脱敏后再发送；`DENY` 可关闭采样，`RAW` 才发送最多十行原值。`MASK/RAW` 都要求租户策略允许、当前操作者逐任务明确授权，并在任务开始、采样前和模型调用前复核冻结的策略版本；撤权会在业务值出域前失败关闭。服务端兼容调用若省略 `sampleDataMode`，仍按 `DENY` 失败关闭。`metadata-completion-v12` 会按本次输入动态固定表 ID、字段 ID 集合和字段数量，并要求表级结果描述业务功能、适用范围和数据粒度，关联候选字段描述业务实体、键角色、方向、唯一性及可空性。超过 5 个待完善字段时，服务按每批最多 5 个字段拆分请求，在单个进程内最多并行 4 批；只有第一批允许返回表级结果，每批样本只保留本批字段，同时通过 `contextColumns` 提供不含业务值的全表紧凑字段概览。各批先独立执行结构和领域校验，全部成功后再以一次整表事务提交；部分批次失败时只保存已通过可信边界的目标 marker，下一轮仅重试缺失字段。标签不设固定数量上限，但只能来自受控的领域、主题、作用、功能、范围、粒度和关联角色词表；模型标签先按首次出现顺序去重，再执行本地枚举校验。Excel/CSV 输入还会通过提示词、输出 Schema 字段说明和服务端领域校验约束字段语言：字段 `businessName` 必须包含中文，中文表头保持其中文含义，英文表头翻译为中文；`businessDescription` 也必须包含中文。兼容层仅剥离完整的 `<think>…</think>` 推理外壳或完整 JSON Markdown 代码块，剩余内容仍必须通过重复键、尾随内容、严格 Schema 和领域一一映射校验；普通说明文字不会被截取为 JSON。服务端校验是最终可信边界，不依赖上游模型对 Schema 方言的完整实现。每个批次的首次输出若存在未知、重复或缺失的资产 ID，或其他领域校验错误，服务会携带安全校验原因和准确目标 ID 清单进行一次结构化纠错；两次调用分别进入通用 AI 审计，领域任务累计实际 Token 用量。
