...
直接调用本接口时，服务只向模型发送表字段技术元数据和现有业务元数据，不发送连接串或密钥。Web 产品中的数据源导入与刷新任务默认选择 `MASK`，按字段数量读取三至十行业务样本并在应用进程内完成格式脱敏后再发送；`DENY` 可关闭采样，`RAW` 才发送同样按字段数量自适应的原始值。`MASK/RAW` 都要求租户策略允许、当前操作者逐任务明确授权，并在任务开始、采样前和模型调用前复核冻结的策略版本；撤权会在业务值出域前失败关闭。

`metadata-completion-v13` 会动态固定表 ID、字段 ID 集合和字段数量，并要求表级结果描述业务功能、适用范围和数据粒度。样本行数随活动字段数量从 10 行线性降低到 3 行：1 个字段取 10 行，达到 50 个字段取 3 行，超过 50 个字段仍取 3 行；样本只在本次调用内使用，不持久化。超过 5 个待完善字段时按每批最多 5 个字段拆分，单进程最多并行 4 批；各批先独立通过结构与语义校验，再以一次整表事务提交。标签只能来自受控的产业、主题、作用、功能、范围、粒度和关联角色词表。业务领域不是标签，由当前用户所属领域和资产 `domain_id` 确定；历史 `领域:* / BUSINESS_DOMAIN` 标签已清理，模型不能生成、复用或推断。
