...
直接调用本接口时，服务只向模型发送表字段技术元数据和现有业务元数据，不发送连接串、密钥或数据样本；经用户授权的数据源导入与刷新流程可额外发送每表最多三行样本。`metadata-completion-v6` 会按本次输入动态固定表 ID、字段 ID 集合和字段数量；模型标签先按首次出现顺序去重，再执行本地枚举校验。CSV 输入还会通过提示词、输出 Schema 字段说明和服务端领域校验约束：字段 `businessName` 必须为小写英文 `snake_case`，`businessDescription` 必须包含中文；安全的 ASCII 空格、连字符、点号和驼峰名称会先规范为 `snake_case`，无法可靠翻译的非 ASCII 名称不会被静默改写。服务端校验是最终可信边界，不依赖上游模型对 Schema 方言的完整实现。首次输出若存在未知、重复或缺失的资产 ID，或其他领域校验错误，服务会携带安全校验原因和准确目标 ID 清单进行一次结构化纠错；两次调用分别进入通用 AI 审计，领域任务累计两次 Token 用量。纠错后仍非法时整次任务失败，不写入建议或正式资产。
