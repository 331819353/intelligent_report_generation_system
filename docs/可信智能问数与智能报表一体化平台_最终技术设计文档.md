# 可信智能问数与智能报表一体化平台——最终技术设计文档

> 文档状态：最终交付版  
> 文档版本：V1.1 Final（新增第五部分：技术补全与一致性修订）  
> 基准日期：2026-08-06  
> 适用仓库：`331819353/intelligent_report_generation_system`  
> 核心技术栈：Golang + React + PostgreSQL/pgvector + NebulaGraph + ECharts + MinIO  
> 文档优先级：本文件作为技术架构、模块边界、数据合同、接口、测试、部署和实施的最终基线；第五部分与前四部分冲突时，以第五部分为准

---

## 0. 文档目标

本文件将总体架构、19 个功能模块、核心 DSL/IR、数据模型、API、Agent Loop、查询编译、报告引擎、评测发布和研发路线合并为一份可直接用于：
...
截至 2026-08-06，当前仓库已具备或部分具备：

- Dataset DSL、查询编译和查询运行时基础。
- AskData 严格公共合同、QuestionUnderstanding、Semantic IR。
- 语义注册表、成员策略、Search 文档、Embedding 和混合检索。
- NebulaGraph 3.8.0 开发部署、Go Adapter 和 GraphPlan 合同。
- Cognition Action、Typed Tool 合同和 Question Run 状态存储。
- 评测数据结构、结果等价和敏感成员 EXACT_ONLY 安全边界。
- React 问数工作台“证据驾驶舱”Mock。

仍需完成的关键生产链路：

- Joint Binder、Beam Search 和置信度校准。
- 完整 GraphPlan 取证和语义 Bundle 验证。
- Semantic IR 到 Query Plan/SQL 的确定性编译。
- 查询计划、基数、成本和结果验证。
- 全部 Tool 实现和真实 Orchestrator Runner。
- Question API/SSE 与 React 真实接线。
- Report V2 领域及问数—报告双向资产。
- 首个真实业务域语义资产和密封黄金集。
- 语义 ACTIVE 激活门禁和双人审批。

当前生产准确率尚未完成密封评测，不得提前宣称达到 95%。
...
| AskData UI Mock | `web/src/pages/AskDataPage.tsx` | 证据驾驶舱交互原型 |

### 2.2 当前关键缺口

```text
internal/askdata/binding      尚需建设 Joint Binder 和校准器
internal/askdata/compiler     尚需建设 IR -> Query DSL 编译
internal/askdata/validator    尚需建设计划、成本和结果校验
tool implementations         多数工具仍需接真实服务
orchestrator runtime         尚需完成真实 Loop 调度
Question API/SSE             尚需接前后端真实运行
Report V2 integration        尚需建设问数与报告双向资产
business semantic assets     需要人工提供和认证
sealed evaluation            尚未完成生产 95% 证明
```
...
```text
M01：internal/access、internal/policy
M02：connector_service、数据源中心
M03：internal/dataset、querycompiler、queryruntime
M04～M08：internal/askdata/registry、dimension、security、migrations 000213+
M09：internal/askdata/search
M10：internal/askdata/graph、deployments/nebula
M11：internal/askdata/understanding
M13 IR 部分：internal/askdata/ir
M15 合同和状态：cognition、toolhost、orchestrator
M18 部分：internal/askdata/evaluation
M16 Mock：web/src/pages/AskDataPage.tsx
```
...
M12：binding
M13：compiler
M14：validator 和真实执行接线
M15：运行时 Runner 和全部 Tool 实现
M16：真实 API/SSE 接线
M17：Report V2 和 reportasset
M18：真实业务黄金集与发布门禁
M19：完整问数可观测和成本治理
```
...
原文将基数与策略混在一个字段中（`SAFE`、`BLOCK`、`ONE_TO_MANY`、`MANY_TO_MANY`、`CERTIFIED_PREAGG` 混用）。拆分为两个独立枚举：
...
- 现有文档中出现的 `CERTIFIED_PREAGG` 统一改为 `PRE_AGGREGATE_REQUIRED`。
...
platform.data_request_events
  request_id, from_state, to_state, actor_user_id, note, created_at
```

API：

```text
POST /api/data-requests                          （支持从 runId 预填）
GET  /api/data-requests
POST /api/data-requests/{id}/submit
POST /api/data-requests/{id}/transition
