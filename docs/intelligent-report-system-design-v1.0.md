...
> **面向多租户组织的数据资产接入、语义建模、指标管理、跨源分析、智能分析决策与报告归档平台。**
...
完成 LLM 业务映射的活动表会对应一张带 `originTableId` 的映射表数据集。它由系统创建并自动发布初始单表版本，其字段与自动提取的原子度量事实只作为指标创作的内部构件和组合数据集输入，不进入指标中心，也不能被报表直接绑定。在指标 AI 编排中，必须基于映射表证据新建普通单源、跨源或派生数据集，不会直接创建指标或原地改造映射表数据集。
...
- 审批通过、不可变发布版本、当前发布指针、审计和内部原子度量事实提取任务必须在同一事务提交，任一步失败都整体回滚；
- 已发布版本不可直接修改；
- 修改后生成新版本；
- 报告版本引用明确的数据集版本；
- 旧版本可回滚但不能被覆盖。
...
指标定义和指标 AI 只能把当前仍为 `PUBLISHED` 的精确数据集版本作为创建、表达式、维度和时间字段来源；人工维护的数据集版本必须来自人工审批，映射表初始版本可以来自可信系统自动发布。可管理的普通数据集草稿可以作为 `MODIFY_DATASET` 的 AI 改造上下文，但不得被指标直接绑定；DWS 保存时已经分类为 `MEASURE` 的原子度量会由程序自动接收并进入指标中心，其他分层的内部原子事实仍只作为 DAG 构件。带 `originTableId` 的映射表数据集及其内部事实在指标 AI 编排中只能作为 `CREATE_DATASET` 的来源证据，不能直接创建指标或成为改造目标。新建或人工改造的草稿保存后仍须提交并通过数据集发布审批。
...
| 数据集发布 | 普通数据集保存草稿后提交审批；审批通过、不可变版本、发布指针、审计和内部原子度量事实提取任务原子提交。映射表初始镜像仅允许可信系统自动发布并独立审计 |
