...
指标定义和指标 AI 只能把当前仍为 `PUBLISHED` 的精确数据集版本作为创建、表达式、维度和时间字段来源；人工维护的数据集版本必须来自人工审批，映射表初始版本可以来自可信系统自动发布。可管理的普通数据集草稿可以作为 `MODIFY_DATASET` 的 AI 改造上下文，但不得被指标直接绑定；数据集发布后提取的原子度量事实只作为内部构件，不进入指标中心或公开指标检索。带 `originTableId` 的映射表数据集及其内部事实在指标 AI 编排中只能作为 `CREATE_DATASET` 的来源证据，不能直接创建指标或成为改造目标。新建或人工改造的草稿保存后仍须提交并通过数据集发布审批。
