...
- `template` 是报告级全局上下文，设计态、预览态、在线查看态和后续 AI 修改流程都读取同一份配置。
- `promptContext` 保存注入 AI 请求的全局提示词约束；标题、正文、配色、画布和分块配置会被确定性合成为补充上下文。
- 渲染器只消费受限设计令牌，包括字体体系、字号、六位十六进制颜色、圆角、内边距和预定义阴影等级；不接受任意 CSS、HTML 或脚本。
- 标题大小和颜色、正文样式、主色/强调色、画布背景/网格、分块背景/边框/圆角/内边距/阴影均由模板生成 CSS 变量，一次修改作用于整份报告。
- `theme` 仅用于兼容早期开放式主题对象，新设计器不再向其中写入全局样式。
...
- 首批正式组件的 `style`、`binding`、`interaction` 和 `refreshPolicy` 已按组件类型收紧；报告级 `template` 已由前后端严格校验，`theme/generation/extensions` 仍保留兼容扩展对象，发布前必须按对应功能再次校验。
