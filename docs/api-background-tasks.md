...
计数不互斥。领域分类阶段为每个精确 ODS 版本创建一次独立 LLM 调用，最多四路并发，
逐 ODS 保存 `warehouse-classification-v5` 检查点，再由服务端合并成领域分类结果。
...
则保留并改为唯一名称。名称扫描和每个
重名组分别保存检查点，重试不会重复设计已完成的 DIM。事实落地按单张 FACT 的小
上下文最多四路并发，并携带同领域去重后的全部已校验 DIM 草稿合同生成 DWD 草稿。
DIM 尚未发布时，事实落地任务以
