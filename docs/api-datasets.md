...
三个任务分别使用租约、最多三次任务级重试和不可变 AI 请求审计。人工提交阶段按领域选择一个代表 ODS，每个领域创建一个父流程和三项阶段任务；ODS 分类、维度设计和事实设计在阶段内部都按单数据集/单产物最多四路并发，并持续续租。重试某个失败阶段会保留已经验证的逐 ODS、逐 DIM 和逐 FACT 检查点，只补缺失分支，再把必要的下游阶段重新排队。单次结果不满足 JSON Schema 或本地安全合同，会在当前小任务内最多追加两次定向修复。
