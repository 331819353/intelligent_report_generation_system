...
- `POST /datasets/trigger-dws-modeling`：遍历当前用户所属领域内全部已发布 DWD，按“单 DWD + 同领域全部 DIM 语义上下文”提交独立 DWS 规划任务。

DIM/DWD 与 DWS 建模不随发布自动启动，只能通过以上入口人工提交。每个入口通过租户隔离的 `SECURITY DEFINER` 函数写入 durable outbox，API 角色不能直接修改 worker 表。worker 异步生成可评审草稿，不直接发布数据集。业务领域唯一取自当前用户所属领域和资产 `domain_id`；`领域:* / BUSINESS_DOMAIN` 历史标签不参与任务分组、检查点或发布校验。维度建模每次点击只创建当前领域的一个父流程；父流程内部按单 ODS、单 DIM 和单 FACT 保存独立检查点并最多四路并发。主题建模按单个 DWD 创建任务，最多四路并发。

响应包含 `eligibleCount`、`enqueuedCount`、`existingCount`、`blockedCount`，以及可选 `blockedReason`。主题建模没有已发布 DWD、但存在 DWD 草稿时返回 `blockedReason=DWD_PUBLICATION_REQUIRED`；页面会提示先提交发布并完成审批。触发函数读取全部当前 DWD，为每个精确发布版本创建一个范围任务；范围中 `dwd` 数组固定只有当前 DWD，`dim` 数组固定携带当前领域全部 DIM 元信息。DIM 只用于 LLM 理解实体、筛选和分组语义，生成的 DWS 物理依赖仍严格指向当前 DWD。当前 DWD 物化尚未变为 `ACTIVE` 时，worker 会进入依赖等待而不是制造失败草稿。多事实比较保留为单独的显式任务能力，不由该默认入口自动触发。
...
人工提交明细建模后，每个领域会创建三个独立且串行依赖的持久任务：`ODS 分类 → 维度建模 → 事实落地`。每次点击“明细建模”都创建新的父批次和三个新的阶段任务；同一精确 ODS 版本只允许一个活动批次，防止重复点击并发入队。任务中心的“重试”恢复原阶段、原父批次及其有效检查点。第一项任务在领域父流程内为每个精确 ODS 版本建立独立 `warehouse-classification-v5` 小上下文，最多四路并行；每个 LLM 只判断当前 ODS 的真实行粒度和可抽取实体，逐表通过校验后保存检查点，服务端再按输入顺序审校领域分类，但不会在 DIM 生成前压掉可能重复的候选。第二项任务以单个维度产物为调用粒度并最多四路并行，既处理独立 DIMENSION/MASTER ODS，也处理 FACT 中声明的实体投影；字段卫生由本地编译器按可信类型和角色强制覆盖。全部候选先各自保存为未发布 DIM 草稿，然后 `warehouse-dim-name-validation-v1` 接收本轮所有 DIM 的名称摘要并识别重名组；只有重名组才读取完整字段和输出粒度，交给 `warehouse-dim-duplicate-validation-v1` 选择保留一张权威表，或确认属于不同实体后分别明确命名。第三项事实落地以单个 FACT ODS 为调用粒度并最多四路并行，每次携带当前 FACT 与同领域去重后的全部已校验 DIM 草稿合同，按原子过程/粒度设计 DWD。DIM 尚未发布时任务以 `PARTIAL / DIM_PUBLICATION_REQUIRED` 结束；全部 DIM 人工发布后自动恢复并把 DWD 依赖重绑定到精确发布版本。
