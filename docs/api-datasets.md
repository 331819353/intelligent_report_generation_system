...
任务使用租约、最多三次任务级重试和不可变 AI 请求审计。同一租户的领取决策由 PostgreSQL 事务级锁串行化，横向扩容多个 Worker 时也不会在首份领域方案完成前并行领取同租户的兄弟任务。单次结果不满足 JSON Schema 或本地安全合同，会一次收集整份方案中最多 64 项字段/输出缺口，并在同一建模尝试内追加一次带全部精确原因的 LLM 定向修复；仍失败才回到持久化重试队列。同一领域短时间内由多个 ODS 发布产生的待处理任务，会在一份完整领域方案成功后合并为 `DOMAIN_PLAN_COALESCED`，避免重复调用 LLM 和重复更新同一批 DWD。
