...
人工提交明细建模后，每个领域会创建三个独立且串行依赖的持久任务：`ODS 分类 → 维度建模 → 事实落地`。每次点击“明细建模”都创建新的父批次和三个新的阶段任务；同一精确 ODS 版本只允许一个活动批次，防止重复点击并发入队。任务中心的“重试”恢复原阶段、原父批次及其有效检查点。第一项任务在领域父流程内为每个精确 ODS 版本建立独立 `warehouse-classification-v5` 小上下文，最多四路并行；每个 LLM 只判断当前 ODS 的真实行粒度和可抽取实体，逐表通过校验后保存检查点，服务端再按输入顺序合并领域分类。第二项任务以单个维度产物为调用粒度并最多四路并行，既处理独立 DIMENSION/MASTER ODS，也处理 FACT 中声明的实体投影；字段卫生由本地编译器按可信类型和角色强制覆盖。第三项事实落地以单个 FACT ODS 为调用粒度并最多四路并行，每次携带当前 FACT 与同领域全部已校验 DIM 草稿合同，按原子过程/粒度设计 DWD。DIM 尚未发布时任务以 `PARTIAL / DIM_PUBLICATION_REQUIRED` 结束；全部 DIM 人工发布后自动恢复并把 DWD 依赖重绑定到精确发布版本。
...
三个任务分别使用租约、最多三次任务级重试和不可变 AI 请求审计。人工提交阶段按领域选择一个代表 ODS，每个领域创建一个父流程和三项阶段任务；ODS 分类、维度设计和事实设计在阶段内部都按单数据集/单产物最多四路并发，并持续续租。单张 ODS 分类输出无效时不会取消其他并行分支，其他分支继续完成并保存检查点；`AI_INVALID_OUTPUT` 会在阶段重试预算内退避恢复，复用已验证检查点后只调用缺失 ODS。逐表分类全部通过后，系统额外执行一次 `warehouse-classification-merge-v1` 领域合并审校：跨表识别同一实体，只保留 `MASTER > 独立 DIMENSION > FACT 内嵌投影` 中最权威、属性最完整的一份 DIM。合并输出再经过确定性同键同类型去重并保存为 `CLASSIFICATION_MERGE` 检查点，后续 DIM/DWD 不再直接消费逐表候选；合并失败只重试合并调用。合并层不得清除唯一存在且已通过逐表合同的 DIM 或 FACT 实体投影。被新结果抑制、且业务键精确匹配最终权威 DIM 的旧自动 DIM，仅在它仍是从未发布/送审、未人工修改且完全无依赖的系统草稿时自动软删除，否则保留并标记人工复核。任务中心重试继续使用 `warehouse-classification-v5`，不会把逐表审计身份降级为旧 v4。单次结果不满足 JSON Schema 或本地安全合同，会先携带最新候选和安全结构诊断定向修复；最后一轮丢弃旧候选，使用干净原始上下文重新生成精简完整 JSON，避免错误答案持续放大上下文。
