...
自动分层建模使用 `warehouse-modeling-v9` 任务合同，并拆成 `warehouse-classification-v1` 与 `warehouse-fact-design-v1` 两类可恢复调用。输入是同一业务领域内已发布 ODS 的完整元数据、字段合同、标签与粒度，不包含样本行。第一类调用只逐表判断 `FACT / DIMENSION / MASTER / OTHER`；分类通过本地校验后立即按 ODS 快照 hash 保存检查点。第二类调用按每张 `FACT` 独立执行，只接收该事实表和已经分类为 `DIMENSION/MASTER` 的表，形成一张保持事实动作粒度、保留关键字段并补充人物/商品等分析属性的 DWD 草稿。`DIMENSION/MASTER` 则由已校验分类确定性形成保留实体键和说明字段的 DIM 草稿。两条分支共享同一个不可变 ODS 快照，可以并行设计和物化；任何一个草稿被人工修改后，后台 hash 栅栏都会跳过覆盖。

每个阶段的结构化结果只有通过版本、角色覆盖、字段血缘、Join、清洗和粒度合同后才写入 `dwd_modeling_checkpoints`。Worker 在每次新 LLM 阶段前续租；进程退出、请求取消或租约回收后，新 Worker 复用分类和已经完成的 FACT 检查点，只调用尚未完成的 FACT。单阶段只允许一次初始调用和一次定向结构修复；网络、超时、429/5xx 仍只由通用 AI Provider 边界有限重试，两层不会互相放大。认证失败、非法请求、拒答、响应过大或定向修复后仍不合格等永久错误直接失败关闭，不再按同一输入重复调用。某次租约只要新增过有效检查点，其后续恢复会重置连续失败额度；没有任何新进展的可重试失败仍受任务 `max_attempts` 限制。

后续建模按层级另行触发，不要求一次模型调用输出全部层：DWD 至少固定一个 ODS 事实输入并可附加正式 DIM；`DWS <- DWD[1..N]`，用于按不同分析视角组合事实并形成结论；`ADS <- DWS` 只为报表、看板、接口或其他明确消费场景预留，当前不自动组合。每一阶段只接收已经发布、可见且具有当前 ACTIVE 物化的精确上游版本，并输出结构化 DSL/DAG。任务结果记录 `DIM → DWD → DWS` 开发流，但发布、物化和下游阶段仍受各自门禁控制。
