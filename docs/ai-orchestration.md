...
第一个任务使用 `warehouse-classification-v2`，在同一业务领域内统一分析全部已发布 ODS 的完整元数据、字段合同、标签与粒度。每张表仍有一个表示其主要行粒度的 `FACT / DIMENSION / MASTER / OTHER` 角色，但角色不再限制产物数量：`FACT` 可以同时声明一组稳定实体键和说明属性，表示从同一 ODS 额外抽取 DIM；因此事实表数量与维度产物数量允许重叠。结果按 ODS 快照 hash 保存检查点。第二个任务使用 `warehouse-dimension-design-v2`，按单个维度产物的小上下文独立设计，包括独立 `DIMENSION/MASTER` ODS 和 FACT 内嵌的实体投影。LLM 只处理实体键与稳定说明属性，补充维度名称、表说明、字段名称、字段说明与结构化标准化方案；本地编译器校验字段边界，执行 `TRIM`、类型对应空值标准值和日期时间转换，并对从事实 ODS 抽取的实体投影执行去重。校验结果逐 DIM 保存检查点。维度任务形成草稿即完成，事实落地随即读取受控 DIM 草稿合同并生成 DWD 草稿，不再等 DIM 发布后才开始设计。DIM 尚未发布时事实任务显示 `WAITING_DIM_PUBLICATION` 和已经生成的 DWD 数量；发布完成后自动把 DWD 依赖切换到精确正式版本，不需要重跑 LLM。

第三个任务使用 `warehouse-fact-design-v3`，按每张 `FACT` 独立执行，只接收该事实 ODS 和第二个任务已加工的 DIM 字段合同。DWD 草稿可暂时引用由本流程生成的精确 DIM 草稿版本；公开保存接口和发布路径仍只接受正式依赖，最终 DWD DAG 必须绑定精确 DIM 发布版本。Worker 会读取现有 DWD 的精确事实 ODS 与 DIM 血缘：来源未变化时直接记为 `UNCHANGED`，事实版本或已使用 DIM 版本变化时只更新受影响的 DWD，新事实补建 DWD，新维度触发现有事实增量复核。DIM 草稿发布但字段合同未变化时，事实检查点保持有效，只执行依赖重绑定，不再次请求 LLM。独立事实分支最多四路并发，每个分支单独保存检查点；单个事实设计失败只形成部分成功，不阻断同领域其他事实。任务全程维持租约心跳，单次模型调用超过基础租约时不会误报 ODS 版本变化。系统生成编码采用 `层级_领域_主题_业务名称` 的可维护物理标识，不使用 UUID 拼接的 `dim_auto_* / dwd_auto_*`。任何自动草稿被人工修改后，后台 hash 栅栏都会跳过覆盖。
...
后续建模按层级另行触发，不要求一次模型调用输出全部层：一个事实 ODS 固定形成自己的 DWD，同时可以先从自身抽取一个实体 DIM；事实设计只允许附加第二步加工的 DIM，避免把未治理的事实属性直接伪装成维度。草稿设计可使用同一受控流程产出的 DIM 草稿合同；DWD/DIM 发布要求 ODS 仍是已发布、可见的精确虚拟来源版本，DWD 附加的 DIM 以及后续 DWS/ADS 上游才要求当前 ACTIVE 数仓物化。主题建模每次人工触发都会遍历全部当前 DWD 和 DIM，先按受治理的分析范围形成主题成员关系，再让 LLM 在主题级小上下文内选择方案。`DWS <- DWD[1..N]`：单事实主题使用有证明条件的分析模板，多事实主题把各 DWD 分别聚合到共同粒度后再关联，绝不把 DWD 与 DWS 固定成一对一。DIM 只作为实体和字段语义的规划上下文，DWS 物理依赖仍严格指向 DWD。`ADS <- DWS` 只为报表、看板、接口或其他明确消费场景预留，当前不自动组合；DWS/ADS 也不在发布或物化时自动产生指标与维度审批项，资产发现由资产管理中心人工触发。任务结果记录 `领域分类 → DIM 标准化 → DWD 事实落地 → 主题 DWS` 开发流，发布、物化和下游阶段仍受各自门禁控制。
