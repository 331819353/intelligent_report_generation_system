...
第一个任务使用 `warehouse-classification-v5`，在同一业务领域父流程内按每个精确 ODS 版本建立独立小上下文，最多四路并发。每次 LLM 只接收当前 ODS 的完整元数据、字段合同、标签与粒度，判断其 `FACT / DIMENSION / MASTER / OTHER` 主角色和可选实体投影；结果逐 ODS 保存检查点，再由服务端按可信输入顺序合并成领域分类。DIM 每行稳定代表一个实体键，只容纳同粒度、同生命周期且函数依赖于该键的说明属性；DWD 每行代表同一业务过程和事件时间含义下的原子事件、交易行或周期快照。本地合同会拒绝把时间键或交易度量放进 DIM 投影。第二个任务使用 `warehouse-dimension-design-v3`，按单个维度产物的小上下文最多四路并发，包括独立 `DIMENSION/MASTER` ODS 和 FACT 内嵌实体投影。LLM 只处理实体键与稳定说明属性，补充维度及字段名称和说明；本地编译器确定性应用字符串、空值和日期卫生合同。事实 ODS 抽取的实体投影继续去重，校验结果逐 DIM 保存检查点。
...
每个阶段的结构化结果只有通过版本、角色覆盖、字段血缘、Join、清洗和粒度合同后才写入 `dwd_modeling_checkpoints`。Join 不再把“类型相同”当作业务关系证据：事实键与维度键 code 必须忽略大小写后完全同名且类型兼容；无法证明的维度扩充会被安全移除。对于已经能由治理元数据唯一证明的合同，编译边界会补齐模型遗漏的事实字段、唯一同名关联、维度说明字段和精确大小写的粒度/时间引用，并丢弃无法编译的可选处理步骤；不会用模糊相似度发明业务关系。Worker 在每次新 LLM 阶段前续租；进程退出、请求取消或租约回收后，新 Worker 会按当前合同重新规范化旧检查点。分类阶段会等待全部并行 ODS 分支收口，单个无效输出不会取消其他分支；已有检查点只读复用，`AI_INVALID_OUTPUT` 在任务 `max_attempts` 内退避重试缺失 ODS。全部逐表结果通过后，再由领域级合并审校 LLM 跨表比较实体键、粒度、属性完整度和来源权威性，产出后续 DIM/DWD 唯一使用的最终分类；本地裁决进一步保证同一明确实体只保留 `MASTER > DIMENSION > FACT 投影` 中的一个 DIM。合并层只能裁决已证明重复的实体，不能删除任何唯一存在且已经通过逐表校验的 DIM/FACT 投影；事实表仍保持 FACT，但重复实体投影会被清空，冗余独立实体源改为 OTHER。最终结果写入独立 `CLASSIFICATION_MERGE` 检查点，合并失败只重试这一层。对于旧任务遗留且业务键与最终权威 DIM 精确匹配的重复自动 DIM，只有未发布、未送审、未人工修改且没有指标、物化或下游依赖的纯草稿才会自动软删除；其余资产保留并要求人工复核。单个 LLM 小任务允许一次初始调用和最多两次结构纠错：第二轮携带最新候选与安全诊断进行增量修复，第三轮移除旧候选并从干净原始上下文重新生成。网络、超时、429/5xx 仍由通用 AI Provider 边界有限重试。认证失败、非法请求、拒答或响应过大等平台级永久错误直接失败关闭；单个 FACT 最终仍不合格则记录精确原因并跳过该 FACT，其余分支继续完成。某次租约只要新增过有效检查点，其后续恢复会重置连续失败额度；没有任何新进展的可重试失败仍受任务 `max_attempts` 限制。
