...
## 3.1 破坏性变更：Report Definition 增加 `zone.order`（2026-08-12）

区域顺序此前借用 `zone.layout.emptyPriority` 表达，而该字段同时决定空区域腾出的
高度优先分配给谁；规范化按升序决定渲染顺序，`CalculateZoneHeights` 按降序决定
分配顺序，两个方向相反。于是一张分析卡片无法既让图表排在上面、又让图表获得腾出
的空间——而这正是「图表 + 结论」卡片需要的组合。`ZONE_REORDER` 的载荷一直叫
`order`，却被写进 `emptyPriority`：它想要的字段本就不存在。

现在 `Zone.Order` 是独立的结构字段（同 `Page.Order`、`Section.Order`），必填且
大于 0；`emptyPriority` 只保留其字面含义。

**影响：变更前落库的报告定义一律无法读取。**

- 草稿与已发布版本都会在 `Decode` 阶段失败：`zones[0]: order must be positive`。
- 已发布版本还会二次失败：`Loader.Load` 会把重新规范化的字节和哈希与制品比对，
  新增字段必然改变两者。
- 没有为定义准备重建通道；`RebuildAllIndexes` 只重建索引，不重写定义。

**处置结论**：按「库中无需保留的报告数据」处理，不提供数据迁移。接手时若本地控制
库仍有旧报告，需清理后重建，而不是尝试就地升级——重写已发布制品会破坏其哈希所
锚定的分享、导出与订阅完整性。
...
18. 报告证据只能由服务端从真实执行推导；接口不接受调用方自带的事实、数值或溯源哈希。
19. 报告结论由模型撰写但不书写任何数字与对象名：模型只输出 `{{fact:…}}`/`{{field:…}}`
    标记，服务端按证据代入取值并计算引用区间，未通过共享事实校验器的结论不落库。
20. 发布前的人工门禁必须是人的声明：客户端不得代发布人确认告警，也不得用自己算得出
