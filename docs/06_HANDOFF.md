...
| B04 语义资产治理 | 后端主链、读取/导入/导出、时间/可加性（含建议补录与批量确认）、词典、KPI、数据质量规则绑定与创作页、指标口径与维度值定义、行级安全策略创作页/覆盖率与主体属性授予页完成 | 正式口径/可加性/行级范围内容待 HUMAN-008/015 |
...
| B12 评测、反馈与运营 | 控制面、密封集轮换、误差预算、反馈工单、主动学习候选与运营页面完成；时间（287 例）与可加性（151 例）专项套件已由平台自有合成清单接入生产引擎，叙述人工评审受理与重算门禁通道就绪 | 正式密封黄金样本、叙述评审正文（HUMAN-016） |
| B13 运维、可观测与成本 | 运行配置页面、配额/成本完成；容量报告新增 `scopeProfile` 与 `POC_NOT_SIGNED` 重算裁决，灾备新增十阶段演练受据与 `DRILL_NOT_SIGNED` 重算裁决 | 目标环境签署（HUMAN-005/013）；本地运行按设计只会得到未签署裁决 |
...
- 身份边界：平台管理员拥有全平台最高权限，可进入全部启用领域；领域管理员可查看和管理所属领域全部信息；普通用户仅访问本人创建、领域内共享或明确分享给本人的内容。认证服务始终绑定明确领域上下文，避免跨领域数据混淆。
...
12. **行级安全策略失败即拒绝。** 阅读者对 `askdata.row_access_policies` 谓词所引用的
    `SUBJECT_ATTRIBUTE` 没有授权时，该策略拒绝其全部行——不放行，也不报错。
    谓词必须至少引用一个主体属性与一个模型字段；属性键由服务端从谓词推导，
    不接受调用方声明。策略随 Release 冻结，在写入、认证与发布门禁三处校验，
    并在编译期作用于**每一个**源节点（锚点与被 Join 的模型各自应用），
    多条策略以 AND 组合，只能收窄。阅读者取值经绑定参数进入 SQL，
    `askdata.resolve_subject_attributes` 只解析会话本人的属性。
    因阅读者取值进入计划，两名阅读者的 plan hash 不同是**必要**属性，
    用于防止跨阅读者复用结果；resolution hash 不含阅读者身份。
13. 不恢复历史 `platform.semantic_*` 运行时与 `000195` 删除的旧报告运行时；Report V2 为独立 bounded context。
14. 数据库迁移编号已使用至 `000341`，新迁移顺延登记，不得复用或重复占用。`000340` 为指标口径说明与维度值定义两个受治理业务上下文字段（SEM-CTX-001）。`000338` 仅允许退休数据源把历史版本凭据替换为 source-specific 撤销墓碑，其他版本字段仍受不可变触发器保护；`000339` 将平台管理员支持队列修正为租户级控制面，业务成员自己的工单仍保持领域隔离。
15. 新页面沿用已确认的蓝 / 白 / 灰设计系统与 1920×1080 基准自主延伸；只有业务口径、权限边界和生产数据变更需要人工治理确认。
16. 用户反馈不直接写生产语义；候选必须经 Owner 审批与回归。
17. **报告主链不得硬依赖模型提供方。** 空白新建、结构编辑、数据绑定、发布、运行、导出与分享
    在未配置 LLM 密钥时必须完整可用；发布评审的裁决由确定性门禁产生，模型只负责叙述，
    缺失或失败时回退为 `DeterministicPublishReview`，绝不因此放宽门禁。
18. **前端统一为 React + 原生 HTML/CSS。** 不得再引入 element-plus、Vue、Ant Design 或其主题层；
    通用按钮走 `web/src/components/AppButton.tsx`，复杂业务控件以原生可访问组件实现，避免第三方样式污染布局。
19. **不得在生产构建中提供设计走查数据。** 所有 `?snapshot=` 分支必须由 `import.meta.env.DEV` 兜住；
    真实模式（idle/live）下的证据面板、执行计划与收据只能展示服务端返回的事实，
    不得回退到示例指标、示例可信度分或示例结论。
20. 报告证据只能由服务端从真实执行推导；接口不接受调用方自带的事实、数值或溯源哈希。
21. 报告结论由模型撰写但不书写任何数字与对象名：模型只输出 `{{fact:…}}`/`{{field:…}}`
    标记，服务端按证据代入取值并计算引用区间，未通过共享事实校验器的结论不落库。
22. 发布前的人工门禁必须是人的声明：客户端不得代发布人确认告警，也不得用自己算得出
    的值充当校验凭据（预览确认已由回显哈希改为明确的 `previewedDesktop`/`previewedMobile`）。
23. **评测套件按「结论属于谁的事实」分类。** 时间与可加性套件断言引擎行为，属平台事实，
...
24. **容量与灾备结论必须重算，不得采信文件。** 压测报告必填 `scopeProfile`
    ...
    任一阶段失败后后续阶段不得记为成功，缺签署输入或超出自身预算固定 `DRILL_NOT_SIGNED`。
    两个裁决都在校验时重算而非比对，因此手工编辑改不出一份已签署的结论。
    本地运行得到未签署裁决是**设计结果**，不是缺陷：开发机结果不是目标环境结论。
    故障注入中「静默丢任务」记为缺证而非延迟告警——吞吐再好也补不回一条丢失的任务。
...
评测专项（无需数据库与外部环境，可直接运行）：

```sh
go run ./cmd/askdata-eval -suite time          # 287 条合成时间用例，阈值 99%
go run ./cmd/askdata-eval -suite additivity    # 151 条合成可加性用例，阈值 100%
go run ./cmd/askdata-eval -narrative-reviews <ledger.json>   # 需 HUMAN-016 的评审账本
```

灾备/容量专项：`./scripts/askdata-postgres-backup.sh`、`./scripts/askdata-postgres-restore.sh`（只接受空目标）、`./scripts/askdata-graph-rebuild.sh`（只指向演练 Space）、`go run ./scripts/loadtest`、`go run ./cmd/askdata-dr-drill -stages`（打印恢复顺序）与 `-receipt <receipt.json>`（校验并重算裁决）；合同见 [04 §6](./04_实施规划与验收终稿.md)。
