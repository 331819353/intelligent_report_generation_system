...
| B04 语义资产治理 | 后端主链、时间/可加性、导入、词典、KPI 完成 | **SEM-READ-001（无对象级读取接口，阻塞语义页面）**、ADD-002、SEM-CTX-001、WEB-007 |
...
12. 数据库迁移编号已使用至 `000299`（Decision、工作箱、会话历史、报告调度、Owner Transfer、运行配置、会话自定义标签、报告发布评审等），新迁移顺延登记，不得复用或重复占用。
