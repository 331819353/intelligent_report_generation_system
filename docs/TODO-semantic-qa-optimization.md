...
- [x] 解析顺序固定为“意图 → 已发布指标 → 指标限域维度值 → 指标绑定的精确数据集版本 → ACTIVE 物化”；证据链继续覆盖成员、维度、指标、数据集、物化和上游数据源。
...
- [x] 今天、昨天、最近 7/30 天、本月、上月、今年和去年等常见中英文时间短语确定性归一化为 preset，并按 IANA 时区在计划创建时冻结为精确边界。
- [x] Top N 形成结构化合同，排名默认 `10 / DESC`，只允许服务端派生指标排序，并用执行行数上限强制收敛。
- [x] 同比、环比和自定义比较窗口形成结构化合同；当前与基准分别使用独立查询 ID、同一指标版本与证据路径执行。
- [x] 跨不同维度的复合成员过滤形成结构化合同；每个成员分别完成有效期、指标兼容与 fanout 复核，原值不持久化。
- [ ] 同一维度多值的 `IN / NOT IN` 语义仍需单独的集合参数与空集合同；在形成同等级证据合同前保持 GAP/澄清。
