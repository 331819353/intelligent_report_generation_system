...
普通数据集发布申请审批通过时，或映射表初始镜像由可信系统自动发布时，不可变 `PUBLISHED` 版本与提取任务在同一数据库事务提交；提交申请或保存草稿都不会提前生成指标候选。后台 Worker 只读取该不可变精确版本，并使用确定性规则为每个可执行数值输出字段生成原子指标候选；这条链路不调用 LLM，因此模型故障不会阻塞发布。规则会优先采用显式度量聚合，其次采用字段语义建议，无法唯一判断时进入 `NEEDS_REVIEW`；聚合数据集、预聚合或字段聚合表达式进入 `BLOCKED`，不会绕过现有执行边界。
