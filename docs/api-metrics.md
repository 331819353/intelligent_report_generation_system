...
| `GET /metric-candidates?status=&datasetId=&limit=&offset=` | 全局 `METRIC:READ` + 全局 `DATASET:READ` | 列出数据集产生精确发布版本后确定性提取的待审核指标候选；普通数据集来自审批发布，映射表初始镜像来自受控系统发布。 |
| `GET /metric-candidates/{id}` | 全局 `METRIC:READ` + 全局 `DATASET:READ` | 查看候选定义、精确来源、证据、假设、告警和阻断原因。 |
| `POST /metric-candidates/{id}/accept` | 全局 `METRIC:MANAGE` + 全局 `DATASET:READ` | 原子接受候选并创建唯一指标草稿，不发布指标。 |
| `POST /metric-candidates/{id}/reject` | 全局 `METRIC:MANAGE` + 全局 `DATASET:READ` | 使用乐观锁拒绝候选并记录原因。 |
| `GET /metrics/semantic-search?q=&limit=&includeCandidates=` | 全局 `METRIC:READ` + 全局 `DATASET:READ` | 用向量与关键词混合检索当前发布指标；可选返回当前发布数据集的待审核候选。 |
...
普通数据集发布申请审批通过时，或映射表初始镜像由可信系统自动发布时，不可变 `PUBLISHED` 版本与提取任务在同一数据库事务提交；提交申请或保存草稿都不会提前生成指标候选。后台 Worker 只读取该不可变精确版本，并使用确定性规则为每个可执行数值度量字段生成原子指标候选。规则固定定义、聚合、允许维度、周期和结构化血缘；随后 LLM 只补全业务名称、说明、口径文字、周期说明、血缘摘要和检索标签，不能改变可执行事实。LLM 不可用或输出非法时自动保存规则降级结果，因此模型故障不会阻塞发布。规则会优先采用显式度量聚合，其次采用字段语义建议，无法唯一判断时进入 `NEEDS_REVIEW`；聚合数据集、预聚合或字段聚合表达式进入 `BLOCKED`，不会绕过现有执行边界。

候选状态为 `READY`、`NEEDS_REVIEW`、`BLOCKED`、`ACCEPTED` 或 `REJECTED`。列表支持状态、数据集和分页筛选，响应包络与指标列表一致。候选记录包含精确 `datasetVersionId`、`dslHash`、完整建议定义、来源字段、规则证据、置信度、假设、告警和阻断原因。

候选还包含 `semantic`：`name`、`description`、`caliber`、`dimensions`、`period`、`periodDescription`、结构化 `lineage`、`lineageSummary`、`tags`、`source`、模型/提示词版本和输入摘要。服务端把这些字段组成不含样本值、SQL 与凭据的版本化文档，异步生成 2560 维向量并写入 PostgreSQL pgvector；重复任务按候选指纹和输入摘要幂等处理。
...
服务端在同一事务锁定候选、复核建议定义及精确数据集版本、创建指标草稿并迁移候选状态。候选身份在租户内唯一绑定一个指标；提交成功但响应丢失时，使用相同候选重试会找回同一草稿。`BLOCKED` 候选不能接受。拒绝请求还必须提供 1–1000 字符的 `reason`。候选是审核队列，不是正式指标；只有接受后创建的草稿才进入指标目录，后续仍须人工校验、试算和发布。
...
```http
GET /api/v1/metrics/semantic-search?q=每个月各区域的销售情况&limit=10&includeCandidates=true
Authorization: Bearer <access-token>
```

默认只返回当前精确 `PUBLISHED` 指标版本，并令 `bindingAllowed=true`。`includeCandidates=true` 会附带状态仍为 `READY/NEEDS_REVIEW`、且来源仍是数据集当前发布版本的候选；候选始终为 `bindingAllowed=false`，只能用于提示用户补建或审核指标。响应包含向量分、关键词分、融合分、embedding 是否就绪以及是否发生关键词降级。远程 embedding 失败不会令接口不可用，但响应的 `degraded=true`。

指标正式发布时，数据库触发器自动为该精确版本生成语义文档：来源候选存在时继承已校验的 LLM 语义，否则从正式指标定义生成确定性语义。旧指标版本、旧数据集版本、已拒绝/已接受候选、停用资产和非当前发布指针均不会进入检索结果。

## 10. LLM 指标创建提案

当前提示词合同版本为 `metric-authoring-v5`。
...
服务端检索调用者具有有效 `READ` 权限且当前状态为 `PUBLISHED` 的内部精确数据集版本、可见逻辑字段和当前发布指标版本，并固定版本 ID 与摘要。普通数据集版本来自发布审批；数据集主对象存在不可变的来源标识 `originTableId` 时，检索上下文会把受控系统发布的初始镜像标记为映射表数据集。它是对应表资产的单表来源镜像，可在已发布字段足够时直接支撑 `CREATE_ON_DATASET`，也可作为新数据集设计的证据，但不能成为 `MODIFY_DATASET` 目标。

服务端还会在操作者同时拥有 `DATASET:READ` 和 `DATASET:MANAGE` 时，把当前普通数据集草稿及其可见逻辑字段放入完全隔离的改造上下文。草稿不会进入可创建指标的数据集候选，也不会作为指标表达式、维度或时间字段的所有者；带 `originTableId` 的映射表草稿即使可见，在指标 AI 编排中也不会被选为原地改造目标。它不会把源数据行、凭据、SQL、物理节点、隐藏字段或外部互联网检索暴露给模型。存在适用行列策略或精确发布依赖不可执行的数据集会被排除。模型应基于需求和授权语义资产尽可能补齐配置，不得要求用户提供数据集、版本或字段的内部 ID。
...
| `CREATE_DATASET` | 不返回指标定义，也不指定现有目标数据集；基于授权的映射表数据集、字段或其他数据集证据，返回由数据集 AI 新建普通数据集的完整业务指令。 |
...
`CREATE_DATASET` 必须令 `targetDatasetId`、`targetDatasetVersionId` 和 `candidateMetricDefinition` 为空，并用 `retrievalEvidence` 证明新建设计来自授权数据；需要关联、派生、补字段或改变映射表结构时必须选此策略，不能把映射表数据集伪装成可原地改造目标。`MODIFY_DATASET` 则必须固定一个授权的普通数据集及精确版本。两种策略的 `datasetInstruction` 都只描述业务目标，不包含 SQL。
