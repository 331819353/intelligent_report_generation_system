-- Keep the bootstrap decision policy aligned with actual active business-domain
-- members. Platform administrators deliberately have no domain membership and
-- therefore cannot be silently installed as business approvers.
...
REVOKE ALL ON FUNCTION decision.sync_default_domain_approval_policy(uuid,uuid) FROM PUBLIC;

CREATE OR REPLACE FUNCTION decision.sync_default_domain_approval_policy_from_membership()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=pg_catalog,decision,platform
...
REVOKE ALL ON FUNCTION decision.sync_default_domain_approval_policy_from_membership() FROM PUBLIC;

DROP TRIGGER IF EXISTS domain_memberships_sync_default_decision_policy
  ON platform.domain_memberships;
CREATE TRIGGER domain_memberships_sync_default_decision_policy
AFTER INSERT OR UPDATE OF status,member_role OR DELETE
ON platform.domain_memberships
FOR EACH ROW EXECUTE FUNCTION decision.sync_default_domain_approval_policy_from_membership();
