...
DO $$
BEGIN
  IF EXISTS(SELECT 1 FROM askdata.release_rollouts) THEN
    RAISE EXCEPTION 'cannot roll back release rollout control while rollout history exists';
  END IF;
END
$$;
...
BEGIN
  IF OLD.status='RETIRED' THEN RAISE EXCEPTION 'retired release is immutable' USING ERRCODE='55000'; END IF;
  ...
  IF NEW.status='SUPERSEDED' AND active_reference_count>0 THEN
    retained_timestamp=clock_timestamp(); NEW.status='RETAINED'; NEW.retained_at=retained_timestamp;
    NEW.retention_until=retained_timestamp+interval '24 months'; NEW.retired_at=NULL;
  ELSIF NEW.status='RETAINED' THEN
    IF OLD.status<>'SUPERSEDED' OR active_reference_count=0 THEN RAISE EXCEPTION 'RETAINED requires a referenced SUPERSEDED release' USING ERRCODE='23514'; END IF;
    retained_timestamp=clock_timestamp(); NEW.retained_at=retained_timestamp;
    NEW.retention_until=retained_timestamp+interval '24 months'; NEW.retired_at=NULL;
  ELSIF NEW.status='RETIRED' THEN
    IF OLD.status NOT IN ('SUPERSEDED','RETAINED') THEN RAISE EXCEPTION 'release can only retire after SUPERSEDED or RETAINED' USING ERRCODE='23514'; END IF;
    IF active_reference_count>0 THEN RAISE EXCEPTION 'RELEASE_RETIRE_BLOCKED: release has active references' USING ERRCODE='23514'; END IF;
    IF OLD.status='RETAINED' AND clock_timestamp()<OLD.retention_until THEN RAISE EXCEPTION 'RELEASE_RETENTION_NOT_EXPIRED: retained release is still within its retention window' USING ERRCODE='23514'; END IF;
    NEW.retained_at=OLD.retained_at; NEW.retention_until=OLD.retention_until; NEW.retired_at=clock_timestamp();
...
CREATE OR REPLACE FUNCTION askdata.enforce_conversation_release_pin()
 ...
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'askdata', 'platform'
AS $function$
...
BEGIN
  IF NOT askdata.tenant_matches(selected_tenant_id)
    ...
    OR (NOT askdata.system_access() AND askdata.current_actor_id() IS NULL) THEN RETURN false; END IF;
  ...
  WHERE conversation.tenant_id=selected_tenant_id AND conversation.domain_id=selected_domain_id
    AND conversation.actor_id=askdata.current_actor_id()
    AND conversation.pinned_release_id=selected_release_id
    AND conversation.pin_source IN ('REPORT_COMPONENT','SAVED_QUESTION') LIMIT 1;
  RETURN COALESCE(release_valid,false);
END
$$;

DROP FUNCTION IF EXISTS askdata.resolve_question_release(uuid,uuid,uuid);
DROP FUNCTION IF EXISTS askdata.release_rollout_bucket(text,uuid);
DROP TABLE askdata.release_rollout_events;
DROP TABLE askdata.release_rollouts;
DROP FUNCTION IF EXISTS askdata.enforce_release_rollout_identity();
