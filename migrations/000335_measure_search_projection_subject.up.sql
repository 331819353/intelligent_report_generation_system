-- The question-facing retrieval contract intentionally exposes both base
-- measures and derived metrics as object_type='METRIC'. The runtime projector
-- already performs that normalization, but the database subject validator only
-- accepted metric_versions. As soon as a later Release replayed SEARCH_INDEX,
-- the first certified measure was rejected and left the Release at 3/4.
--
-- Keep the public retrieval vocabulary stable and validate a METRIC search
-- document against either governed source table. UUID identities remain the
-- immutable object_version_id, so no ownership or sensitivity rule is relaxed.
BEGIN;
  ...
  CASE NEW.object_type
    ...
    WHEN 'METRIC' THEN
      SELECT EXISTS(
        ...
        UNION ALL
        SELECT 1 FROM askdata.measures
        WHERE tenant_id=NEW.tenant_id AND domain_id=NEW.domain_id
          AND id=NEW.object_version_id AND status='CERTIFIED'
      ) INTO subject_valid;
    WHEN 'DIMENSION' THEN
