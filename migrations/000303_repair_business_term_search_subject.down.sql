...
BEGIN
  CASE NEW.object_type
    ...
    WHEN 'BUSINESS_TERM' THEN
      SELECT EXISTS(
        SELECT 1
        FROM askdata.business_terms AS identity
        JOIN askdata.business_term_versions AS version
          ON version.business_term_id=identity.id
         AND version.tenant_id=identity.tenant_id
         AND version.domain_id=identity.domain_id
        WHERE identity.tenant_id=NEW.tenant_id AND identity.domain_id=NEW.domain_id
          AND version.id=NEW.object_version_id
