...
DELETE FROM askdata.search_recall_audits WHERE doc_type='SEMANTIC_MODEL';
DELETE FROM askdata.search_query_samples WHERE doc_type='SEMANTIC_MODEL';
...
ALTER TABLE askdata.search_query_samples
  ...
  ADD CONSTRAINT search_query_samples_doc_type_check CHECK(doc_type IN (
    'METRIC','DIMENSION','MEMBER','BUSINESS_TERM','CERTIFIED_EXAMPLE','REPORT_ASSET'
  ));
