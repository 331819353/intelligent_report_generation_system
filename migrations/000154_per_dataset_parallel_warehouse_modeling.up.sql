-- 把后台 LLM 粒度统一为可独立恢复的小任务：
--   ODS 分类：每个精确 ODS 版本一个 classification-v5 检查点；
--   DIM/DWD：继续按单个维度产物、单个事实 ODS 最多四路并行；
--   DWS：每个精确 DWD 版本一个任务，携带同领域全部 DIM 语义上下文。
-- 多事实比较不再混入默认主题建模，保留给后续显式跨事实入口。
...
BEGIN
  ...
  IF definition=original
    ...
    RAISE EXCEPTION '无法升级逐 ODS 分类 prompt 合同';
  END IF;
  EXECUTE definition;
END
$migration$;
...
SET status='SKIPPED',
    error_code='MODELING_SCOPE_SUPERSEDED',
    error_message='默认主题建模已改为逐 DWD 并行任务，请重新点击主题建模',
    lease_owner='',lease_token=NULL,lease_expires_at=NULL,
    completed_at=now(),updated_at=now()
WHERE prompt_version='dws-group-planning-v2'
...
COMMENT ON FUNCTION platform.trigger_manual_dws_modeling(uuid) IS
  '逐个当前发布 DWD 创建独立主题建模任务；每个任务携带当前领域全部 DIM 语义上下文';
