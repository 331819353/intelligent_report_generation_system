-- ODS is now a first-class warehouse materialization target. Its soft-delete
-- path must use the same leased cleanup outbox as the downstream layers.
...
COMMENT ON TABLE platform.dataset_materialization_cleanup_jobs IS
  'ODS/DIM/DWD/DWS/ADS 数据集删除后，由仓库 worker 清理稳定视图、历史视图和物理表的跨库 outbox';

-- PRE_AGGREGATED is the only DWS contract allowed to freeze a physical source
-- directly. Ordinary DWS still accepts only governed DWD/DIM inputs.
...
COMMENT ON FUNCTION platform.enforce_build_run_required_input_layers() IS
  '延迟校验构建输入根层：ODS<-SOURCE、DIM<-ODS、DWD<-ODS、DWS<-SOURCE|DWD|DIM、ADS<-DWS';
