-- A failed metadata import can create a system-owned ODS draft before the
-- table is fully enriched. If that draft is deleted and a later import
-- succeeds, SYSTEM_MAPPED_REGENERATE must also accept the provable
-- never-published state; the previous contract required published history.
      ...
      WHERE dataset.id=candidate.dataset_id
        ...
        AND (
          ...
          OR (
            ...
            AND candidate.source_draft_record_version>0
            AND dataset.version=candidate.source_draft_record_version+1
            AND EXISTS(
              SELECT 1 FROM platform.dataset_versions AS draft
            ...
            AND NOT EXISTS(
              SELECT 1 FROM platform.dataset_draft_revisions AS revision
              WHERE revision.tenant_id=candidate.tenant_id
                AND revision.dataset_id=candidate.dataset_id
                AND revision.draft_version_id=candidate.source_draft_version_id
                AND revision.operation_type='ROLLBACK'
            ...
            AND NOT EXISTS(
              SELECT 1 FROM platform.audit_logs AS audit
              WHERE audit.tenant_id=candidate.tenant_id
                AND audit.resource_type='DATASET'
                AND audit.resource_id=candidate.dataset_id::text
