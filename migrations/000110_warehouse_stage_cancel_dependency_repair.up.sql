-- 中止任一阶段时，必须终结同一流程内所有仍活动的阶段；重试则恢复
-- 选中阶段、下游阶段，以及因整条流程中止而未成功的上游阶段。
...
COMMENT ON FUNCTION platform.cancel_dwd_modeling_stage_task(uuid,uuid) IS
  '中止一个建模阶段并以租约栅栏终结同一领域流程的全部未完成阶段';
