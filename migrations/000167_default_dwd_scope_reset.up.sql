-- 默认明细建模复用已完成 DIM 工作流时，不能继承该工作流上一次人工选择
-- 保存的 FACT/DIM 限制；同时只允许复用覆盖当前全部已发布 ODS 的 DIM 批次。
DO $migration$
DECLARE
  definition text;
  original text;
BEGIN
  ...
  IF definition=original
    ...
    RAISE EXCEPTION '无法启用默认明细建模范围重置';
  END IF;
  EXECUTE definition;
END
$migration$;
...
BEGIN
  ...
  IF enqueued_count>0 THEN
    ...
    SET fact_source_dataset_ids=CASE
          WHEN selected_dataset_ids IS NULL THEN NULL
          ELSE selected_ods
        END,
        fact_dimension_dataset_ids=CASE
          WHEN selected_dataset_ids IS NULL THEN NULL
          ELSE selected_dim
        END,
        updated_at=now()
...
COMMENT ON FUNCTION platform.trigger_manual_dwd_modeling(uuid) IS
  '默认明细建模只复用覆盖当前全部 ODS 的最新成功 DIM 批次，并在激活时清除历史人工选择范围';
COMMENT ON FUNCTION platform.trigger_manual_dwd_modeling(uuid,uuid[]) IS
  '空选择执行默认全量明细建模并清除旧范围；非空选择在同一完整 DIM 批次上限制 FACT 与 DIM 规划范围';
