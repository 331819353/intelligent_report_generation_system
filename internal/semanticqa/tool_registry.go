...
func newQuestionToolRegistry() *QuestionToolRegistry {
	...
	items := []QuestionToolSpec{
		{Name: "search_semantic_objects", CanonicalName: "search_semantic_objects", Description: "按 Span、对象类型和领域检索活动发布中的语义对象", Parameters: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["query","objectTypes"],"properties":{"query":{"type":"string","minLength":1,"maxLength":512},"objectTypes":{"type":"array","minItems":1,"maxItems":8,"items":{"type":"string"}},"topK":{"type":"integer","minimum":1,"maximum":20}}}`), AllowedStates: []QuestionState{QuestionStateContextReady, QuestionStateValidating}},
		...
		{Name: "lookup_dimension_values", CanonicalName: "lookup_dimension_values", Description: "在固定维度和有效期内解析规范值 ID", Parameters: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["dimensionId","aliasKey"],"properties":{"dimensionId":{"type":"string"},"aliasKey":{"type":"string","minLength":1,"maxLength":512},"effectiveAt":{"type":"string"}}}`), AllowedStates: []QuestionState{QuestionStateValidating}},
		...
		{Name: "get_certified_examples", CanonicalName: "get_certified_examples", Description: "读取相同业务域和对象范围的认证问句及 Semantic IR 摘要", Parameters: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["objectIds"],"properties":{"objectIds":{"type":"array","maxItems":16,"items":{"type":"string"}},"limit":{"type":"integer","minimum":1,"maximum":3}}}`), AllowedStates: []QuestionState{QuestionStateValidating}},
		...
		{Name: "probe_join_cardinality", CanonicalName: "probe_join_cardinality", Description: "以固定模板探测认证 Join 边的唯一率、重复倍率和空键率", Parameters: json.RawMessage(`{"type":"object","additionalProperties":false,"required":["graphPlanId","edgeId"],"properties":{"graphPlanId":{"type":"string"},"edgeId":{"type":"string"}}}`), AllowedStates: []QuestionState{QuestionStateValidating}, WarehouseAccess: true},
