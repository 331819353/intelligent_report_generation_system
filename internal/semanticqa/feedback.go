...
func (store *PostgresStore) UpsertQueryFeedback(
	...
	err = database.WithTenantTx(ctx, store.pool, tenantID, func(tx pgx.Tx) error {
		row := tx.QueryRow(ctx, `INSERT INTO platform.semantic_query_feedback(
				tenant_id,query_plan_id,actor_id,rating,comment
			)
			SELECT platform.current_tenant_id(),plan.id,$2::uuid,$3,$4
			...
			RETURNING id::text,query_plan_id::text,rating,comment,
				created_at::text,updated_at::text`,
			queryPlanID, actorID, input.Rating, input.Comment,
		)
		if scanErr := row.Scan(
			&item.ID, &item.QueryPlanID, &item.Rating, &item.Comment,
