...
func (s *Service) Publish(ctx context.Context, tenantID, actorID, id, idempotencyKey string, input PublishInput) (VersionRecord, error) {
	...
	if s.validator == nil {
		return VersionRecord{}, ErrPublishUnavailable
	}
	current, err := s.store.Get(ctx, tenantID, id)
	if err != nil {
	...
	if err != nil {
		return VersionRecord{}, publicationFailure("dsl", "PUBLISH_DSL_INVALID", "当前草稿无法重新通过 DSL 校验")
	}
	if prepared.DSLHash != current.DSLHash || prepared.PlanHash != current.PlanHash {
		return VersionRecord{}, publicationFailure("dsl", "PUBLISH_DERIVATION_MISMATCH", "当前草稿与服务端派生摘要不一致")
	}
	issues := unconfirmedJoinIssues(prepared.Document)
	if len(issues) > 0 {
		return VersionRecord{}, &PublicationValidationError{Issues: issues}
	}
	...
	if err != nil {
		...
		return VersionRecord{}, publicationFailure("executionPolicy", "PUBLISH_QUERY_FAILED", "发布前查询试跑失败")
	}
	if warningIssues := publicationWarningIssues(prepared.Document, result.Warnings); len(warningIssues) > 0 {
		return VersionRecord{}, &PublicationValidationError{Issues: warningIssues}
	}
	record, err := s.store.Publish(ctx, tenantID, actorID, id, PublishPlan{
...
func unconfirmedJoinIssues(document Document) []PublicationIssue {
	...
	for index, join := range document.Joins {
		if !join.ManualConfirmed {
			issues = append(issues, PublicationIssue{Path: fmt.Sprintf("joins[%d].manualConfirmed", index), Code: "JOIN_CONFIRMATION_REQUIRED", Reason: "发布前必须确认 Join 字段和基数"})
		}
	}
	return issues
}
