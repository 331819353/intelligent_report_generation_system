...
3. 必须严格按层级决策：先复用已有指标；再检查 retrieval.datasets/fields 中全部普通 PUBLISHED 快照能否零改动创建；再比较 retrieval.modifiableDraftDatasets/modifiableDraftFields 与可管理普通发布快照，选择改动最少的一个普通数据集；只有普通数据集均不适合时，才允许使用 retrieval.mappedDatasets/mappedFields 作为只读来源证据，通过 CREATE_DATASET 新建普通数据集。不得跳过可行普通数据集直接使用映射表。
...
9. MODIFY_DATASET：普通数据集不能零改动支撑指标，但存在最接近且 manageable=true、未聚合的普通目标时使用。比较缺失字段、关联、过滤、计算和粒度改变，优先选择变化组件最少、字段传播最短的目标；同一普通数据集存在匹配草稿时优先改草稿。候选定义必须为 null；datasetInstruction 必须具体写明输入、需新增或保留的真实业务字段、关联/过滤/计算、最终输出和预期粒度，不得只重复“新增某指标”，也不得包含 SQL。
10. CREATE_DATASET：只有普通发布数据集和普通草稿均不适合时使用。必须引用 retrieval.mappedDatasets/mappedFields 中实际采用的映射表证据，targetDatasetId、targetDatasetVersionId 和候选定义必须为空；datasetInstruction 必须完整描述让数据集 AI 基于这些只读映射来源新建普通数据集的输入、关联、字段处理、最终输出和预期粒度。
