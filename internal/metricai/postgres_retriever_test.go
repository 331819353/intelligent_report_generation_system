...
type retrievalQueryerStub struct {
	metricRows, datasetRows, draftDatasetRows *retrievalRowsStub
	queries                                   []string
	arguments                                 [][]any
...
func TestPostgresRetrieverReturnsPermissionScopedMinimalExactContext(t *testing.T) {
	...
	if len(queryer.validations) != 1 || len(queryer.queries) != 3 {
...
func TestPostgresRetrieverMarksMappedDatasetFromOriginTableFlag(t *testing.T) {
	...
	if len(result.Datasets) != 1 || !result.Datasets[0].Mapped {
...
func TestPostgresRetrieverSQLContainsEffectiveReadAndPolicyFences(t *testing.T) {
	for name, sql := range map[string]string{"datasets": authorizedDatasetsSQL, "metrics": authorizedMetricsSQL} {
	...
	for _, forbidden := range []string{"platform.data_sources", "platform.table_assets", "sample_rows", "credential"} {
		if strings.Contains(strings.ToLower(authorizedDraftDatasetsSQL), forbidden) {
			t.Fatalf("draft dataset query reaches forbidden source data %q", forbidden)
		}
	}
}
...
func TestPostgresRetrieverSharesDatasetBudgetAcrossPublishedAndDraftSnapshots(t *testing.T) {
	...
	if len(result.Datasets) != 20 || len(result.ModifiableDraftDatasets) != maxDatasets-20 {
		t.Fatalf("dataset budget was not shared: published=%d draft=%d", len(result.Datasets), len(result.ModifiableDraftDatasets))
	}
	if len(result.Datasets)+len(result.ModifiableDraftDatasets) != maxDatasets {
		t.Fatalf("combined dataset count = %d, want %d", len(result.Datasets)+len(result.ModifiableDraftDatasets), maxDatasets)
	}
}
