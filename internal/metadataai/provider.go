...
表标签还必须恰好包含一个“层级:”标签，判断这张物理表当前已经处于的数仓层级，而不是它将来应该被加工到哪一层。ODS 与 DWD 的区别不看表名前缀，只看两条证据：一是有没有补充高频维度信息，二是有没有做默认值填充。“层级:ODS”是原始数据：只有业务字段和维度外键 ID（customer_id、store_id、product_id 之类），没有冗余出客户名称、门店名称、商品名称/类目、地区名称等高频维度属性；样本中可能大量空值、状态码原样保留、没有默认值填充。“层级:DWD”是已清洗的明细：在明细粒度上冗余保留了高频维度属性（ID 旁边同时带名称/类目/等级等描述列）、空值已按默认值填充（0、“未知”、占位日期等）、字段口径规范统一。只有同时具备这两类证据才判 DWD，否则一律判 ODS，即使表名带 fact_ 或 dwd_ 前缀。客户、商品、门店、组织、日历等实体主数据或维度表选“层级:DIM”；每行是按日期/客户/渠道等维度汇总统计的聚合结果（常见 agg_/dws_/summary/统计/汇总/指标 等证据、度量列为汇总值）选“层级:DWS”；面向报表、看板或应用直接消费的结果宽表（常见 ads_/report/rpt/看板/驾驶舱 等证据）选“层级:ADS”。证据不足时按“ODS 优先于 DWD、DWD 优先于 DWS”的保守顺序选择，绝不能仅凭有数值字段就判定为 DWS。字段标签不得携带“层级:”标签。
