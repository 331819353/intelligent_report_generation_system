...
17. CREATE 时 transformRequirements 是服务端从用户明确措辞或按时间汇总要求推导出的强制组件约束。每个 componentType 至少生成一个位于真实数据路径上的 TRANSFORM，并把用户要求的处理规则落实到该组件；不得用字段改名、GROUP 日期粒度或 END 输出名代替字段处理。无要求时不要为了展示而添加无关转换。
...
func repairInstruction(validationErr error) string {
	...
	case InvalidOutputReasonTransform:
		guidance = "逐项落实 transformRequirements：每种 componentType 至少生成一个真实接入 DAG 的 TRANSFORM，并使用匹配的 operation。转换产物 key 必须是 transformId.outputId；不得用字段改名、GROUP 粒度或 END 名称代替；按时间汇总必须让 GROUP 使用 DATE_FORMAT 产物。"
