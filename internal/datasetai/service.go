...
19. 返回 READY 前必须逐项自检 fieldChanges：每个 FINAL_OUTPUT 从其 node 到 end 的路径上，每经过一个现有或新增 GROUP，都必须在 groupUses 中恰好声明一次 DIMENSION 或 METRIC；非日期维度的 grouping 必须为空，只有日期时间字段可以填写时间粒度。若字段角色、时间粒度或聚合方式不能从 instruction/hints 唯一确定，返回 CLARIFY，不能输出一个会被本地校验拒绝的 READY。
...
17. CREATE 时 transformRequirements 是服务端从用户明确措辞推导出的强制组件约束。每个 componentType 至少生成一个位于真实数据路径上的 TRANSFORM，并把用户要求的处理规则落实到该组件；不得用字段改名、GROUP 日期粒度或 END 输出名代替字段处理。无要求时不要为了展示而添加无关转换。
...
func repairInstruction(validationErr error) string {
	...
	case InvalidOutputReasonTransform:
		guidance = "逐项落实 transformRequirements：每种 componentType 至少生成一个真实接入 DAG 的 TRANSFORM，并使用匹配的 operation。转换产物 key 必须是 transformId.outputId；不得用字段改名、GROUP 粒度或 END 名称代替。"
...
func intentRepairInstruction(validationErr error) string {
	...
	switch metadata.ReasonCode {
	case InvalidOutputReasonGroup:
		guidance = "逐个检查 FINAL_OUTPUT 字段从 node 到 end 经过的全部 GROUP，并在 groupUses 中对每一级恰好声明一次 DIMENSION 或 METRIC；非日期维度的 grouping 留空，时间粒度只用于日期时间字段。无法唯一确定角色或聚合方式时必须返回 CLARIFY。"
	case InvalidOutputReasonAggregationField:
