...
func proposalOutputSchema(catalog []CatalogTable) map[string]any {
	...
	transformRule := strictObject([]string{"id", "operation", "inputKeys", "output", "unit", "dateSource", "startDateSource", "endDateSource", "targetType", "matchValue", "thenMode", "thenValue", "elseMode", "elseValue", "conditionOperator", "conditionValues", "fallbackMode", "fallbackValue", "separator", "precision", "start", "length", "searchValue", "replacementValue", "replaceSourceKey"}, map[string]any{
		"id":                identifier,
		"operation":         map[string]any{"type": "string", "enum": []string{"CURRENT_DATE", "DATE_DIFF", "DATE_EXTRACT", "DATE_START", "DATE_END", "DATE_FORMAT", "CAST", "ADD", "SUBTRACT", "MULTIPLY", "DIVIDE", "ROUND", "ABS", "FLOOR", "CEIL", "CONCAT", "COALESCE", "CASE", "SUBSTRING", "TRIM", "UPPER", "LOWER", "REPLACE"}},
		"inputKeys":         map[string]any{"type": "array", "maxItems": 16, "items": fieldKey},
		"output":            transformOutput,
		"unit":              map[string]any{"type": "string", "enum": []string{"", "YEAR", "MONTH", "QUARTER", "WEEK", "DAY", "WEEKDAY", "DAY_OF_YEAR"}},
		"dateSource":        map[string]any{"type": "string", "enum": []string{"", "FIELD", "CURRENT_DATE"}},
		"startDateSource":   map[string]any{"type": "string", "enum": []string{"", "FIELD", "CURRENT_DATE"}},
		"endDateSource":     map[string]any{"type": "string", "enum": []string{"", "FIELD", "CURRENT_DATE"}},
		"targetType":        map[string]any{"type": "string", "enum": []string{"", "STRING", "INTEGER", "DECIMAL", "BOOLEAN", "DATE", "DATETIME"}},
		"matchValue":        shortText(500),
		"thenMode":          map[string]any{"type": "string", "enum": []string{"", "LITERAL", "CURRENT_DATE"}},
		"thenValue":         shortText(500),
		"elseMode":          map[string]any{"type": "string", "enum": []string{"", "LITERAL", "CURRENT_DATE"}},
		"elseValue":         shortText(500),
		...
		"fallbackMode":      map[string]any{"type": "string", "enum": []string{"", "LITERAL", "FIELD", "CURRENT_DATE"}},
