...
export function ReportHeader({
  ...
  const advanced = advancedByStyle[style] ?? style === '03'
  const displayFilters = filters.length ? filters.slice(0, 4) : previewFilters
  const readonlyPreview = filters.length === 0
  const chips = useMemo(() => displayFilters.map((filter, index) => ({
    id: filter.id, label: filter.fieldRef.field, value: filterValueText(filter, values[filter.id], index),
  })), [displayFilters, values])
  ...
  return <section className={`report-designed-header is-style-${style} ${compact ? 'is-compact' : ''} ${advanced ? 'is-advanced' : ''} ${applied ? 'is-applied' : ''}`.trim()} aria-label="报告头">
    ...
    <div className="report-designed-filter-card">
      <header><div><span><Funnel size={16} weight="duotone" /></span><p><strong>筛选条件</strong><small>调整报告分析范围</small></p></div>
        <button type="button" className="report-designed-advanced-toggle" aria-expanded={advanced} onClick={() => setAdvancedByStyle(current => ({ ...current, [style]: !advanced }))}><SlidersHorizontal size={14} />高级筛选<CaretDown size={12} /></button>
      </header>
      <fieldset className="report-designed-filter-fields" disabled={!onChange || readonlyPreview}>
        {displayFilters.map(filter => <FilterControl key={filter.id} filter={filter} value={values[filter.id]} onChange={value => change(filter.id, value)} />)}
      </fieldset>
      ...
      <div className="report-designed-advanced-panel" aria-hidden={!advanced}>
        <header><div><strong>高级筛选</strong><small>组合多个条件缩小分析范围</small></div><span>{displayFilters.length} 个可用字段</span></header>
        <fieldset disabled={!onChange || readonlyPreview}>{displayFilters.map(filter => <FilterControl key={`advanced-${filter.id}`} filter={filter} value={values[filter.id]} onChange={value => change(filter.id, value)} />)}</fieldset>
      </div>
    </div>
  </section>
}
