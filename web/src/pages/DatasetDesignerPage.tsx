...
import { buildDatasetDSL, buildPreviewParameters, datasetAPI, joinCardinalityForType, type AssetColumn, type AssetTable, type CalculatedField, type DatasetDraft, type DatasetPreview, type DatasetRecord, type DesignerNode, type FieldOption, type FilterOption, type JoinOption, type ParameterOption, type PublishedVersionRecord, type PublishedVersionSummary, type PublishDatasetInput, type VersionUsage } from '../lib/datasets'
...
async function hydrateDraft(record: DatasetRecord, tables: AssetTable[]): Promise<DatasetDraft> {
  ...
  const joins: JoinOption[] = list(dsl.joins).map(object).map(raw => {
    ...
    return { id: text(raw.id), leftNodeId: text(raw.leftNodeId), rightNodeId: text(raw.rightNodeId), leftField: first.leftField, rightField: first.rightField, joinType, cardinality: joinCardinalityForType(joinType), manualConfirmed: Boolean(raw.manualConfirmed), conditions }
  })
...
export function DatasetDesignerPage() {
  ...
  const addTable = async (table: AssetTable) => {
    ...
    try {
      ...
      const joins = nodes.length > 1 ? [...draft.joins, { id: `join_${nodes.length - 1}_${nodes.length}`, leftNodeId: nodes[nodes.length - 2].id, rightNodeId: id, leftField: nodes[nodes.length - 2].columns[0]?.columnName ?? '', rightField: columns[0]?.columnName ?? '', joinType: 'INNER', cardinality: joinCardinalityForType('INNER'), manualConfirmed: false }] : draft.joins
  ...
  const publishDataset = async () => {
    ...
    try {
      ...
      publishAttempt.current = null
      setPublishOutcomeUnknown(false)
      setMessage(`发布审批已提交 · ${request.id} · 审批通过后启动加工`)
    } catch (cause) {
      publishAttempt.current = null
      setPublishOutcomeUnknown(false)
...
function JoinEditor({ join, nodes, onChange }: { join: JoinOption; nodes: DesignerNode[]; onChange: (value: JoinOption) => void }) {
  const left = nodes.find(node => node.id === join.leftNodeId)
  const right = nodes.find(node => node.id === join.rightNodeId)
  const update = (patch: Partial<JoinOption>) => {
  ...
  return <article className="join-editor"><strong>关联 {left?.alias} → {right?.alias}</strong><select value={join.leftField} onChange={event => update({ leftField: event.target.value, manualConfirmed: false })}>{left?.columns.map(column => <option key={column.id}>{column.columnName}</option>)}</select><span>=</span><select value={join.rightField} onChange={event => update({ rightField: event.target.value, manualConfirmed: false })}>{right?.columns.map(column => <option key={column.id}>{column.columnName}</option>)}</select><select value={join.joinType} onChange={event => update({ joinType: event.target.value, manualConfirmed: false })}><option>INNER</option><option>LEFT</option><option>RIGHT</option><option>FULL</option></select><label className="join-confirm"><input type="checkbox" checked={join.manualConfirmed} onChange={event => update({ manualConfirmed: event.target.checked })} />已核对关联</label></article>
}
