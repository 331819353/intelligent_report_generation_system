...
test('新建文件数据源只上传创建，在新增数据表时解析预览并提交 Sheet 映射', async () => {
	...
	const created = source({ id: 'source-excel', code: 'analysis_xlsx', name: 'analysis', type: 'EXCEL', status: 'DRAFT', config: {}, fileAssetId: 'file-1', version: 1 })
	const active = { ...created, status: 'ACTIVE' as const, version: 2 }
	const create = vi.spyOn(dataSourceAPI, 'create').mockResolvedValue(created)
	const testConnection = vi.spyOn(dataSourceAPI, 'test').mockResolvedValue({ serverVersion: 'Excel xlsx', latencyMs: 1 })
	vi.spyOn(dataSourceAPI, 'get').mockResolvedValue(active)
	...
	expect(create).toHaveBeenCalledWith({ code: 'analysis_xlsx', name: 'analysis', type: 'EXCEL', fileAssetId: 'file-1' })
	...
	expect(within(picker).getByText('可重新映射')).toBeInTheDocument()
	expect(within(picker).getByRole('checkbox', { name: /Sales表头第 1 行/ })).toBeEnabled()
...
test('重复上传同名文件时覆盖原文件资产版本且不重复创建数据源', async () => {
	...
	const refreshed = { ...existing, version: 6 }
