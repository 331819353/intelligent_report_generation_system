...
test('展示已有数据源并可查看不含密码的连接详情', async () => {
  ...
  const dialog = screen.getByRole('dialog', { name: '数据源详情' })
  expect(within(dialog).getByText('mysql.internal')).toBeInTheDocument()
  expect(within(dialog).getByText('report_reader')).toBeInTheDocument()
  expect(within(dialog).getByText('已安全保存，不可查看')).toBeInTheDocument()
...
test('点击数据源卡片展示全部表结构并提供管理操作', async () => {
  ...
  await user.click(within(cardFor('销售业务库')).getByRole('button', { name: '查看销售业务库详情和表结构' }))
  const dialog = await screen.findByRole('dialog', { name: '数据源详情' })

  expect(tables).toHaveBeenCalledWith('source-1')
  expect(within(dialog).getByRole('button', { name: '新增数据表' })).toBeEnabled()
  expect(within(dialog).getByRole('button', { name: '测试连接' })).toBeEnabled()
