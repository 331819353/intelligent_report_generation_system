...
export function AddToReportDialog({ open, snapshot, run, onClose, onApplied }: AddToReportDialogProps) {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (snapshot) {
        const fixtures: ReportAsset[] = [
          { id: '00000000-0000-4000-8000-000000000201', code: 'RPT-OPERATIONS-2026', name: '2026年经营分析月报', reportType: 'REPORT', ownerUserId: 'snapshot-owner', ownerName: '王敏', lifecycle: 'CHANGED', currentVersionNo: 7, draftRevisionNo: 12, unpublishedChanges: 2, updatedAt: '2026-08-10T09:30:00+08:00', visibleCount: 18, editableCount: 4, shared: false, allowedActions: ['VIEW', 'EDIT', 'PUBLISH'], previewKind: 'operations' },
          { id: '00000000-0000-4000-8000-000000000202', code: 'RPT-CHANNEL-2026', name: '渠道健康度分析报告', reportType: 'REPORT', ownerUserId: 'snapshot-owner', ownerName: '王敏', lifecycle: 'DRAFT_ONLY', draftRevisionNo: 4, unpublishedChanges: 1, updatedAt: '2026-08-09T16:20:00+08:00', visibleCount: 9, editableCount: 2, shared: false, allowedActions: ['VIEW', 'EDIT', 'PUBLISH'], previewKind: 'channel' },
        ]
  ...
  const createIntent = async () => {
    ...
    try {
      ...
      let result = await questionAPI.addToReport({ runId: run.runId, reportId: selectedID, runVersion: run.recordVersion })
      for (let attempt = 0; result.status === 'PENDING' && attempt < 6; attempt += 1) {
  ...
  const confirm = async () => {
    ...
    setBusy(true); setError('')
    try {
      const result = snapshot ? { ...intent, status: 'APPLIED' as const } : await questionAPI.confirmAddToReport(intent.intentId, intent.previewHash)
      setIntent(result)
      if (result.status === 'APPLIED') onApplied(result.reportId)
    } catch (cause) {
      setError(mapAskDataError(cause).message)
  ...
  return <div className="ask-dialog-overlay" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose() }}>
    <section className="ask-add-report-dialog" role="dialog" aria-modal="true" aria-labelledby="ask-add-report-title">
      ...
      {intent?.status === 'PENDING_CONFIRMATION' && <div className="ask-report-preview"><CheckCircle size={24} weight="fill" /><div><strong>变更预览已生成</strong><p>将在“{selected?.name ?? '所选报告'}”中新建一个受治理图表组件，并固定当前 Run、语义版本和证据。</p><dl><div><dt>组件类型</dt><dd>渠道贡献分析</dd></div><div><dt>目标位置</dt><dd>报告末尾 · 新章节</dd></div><div><dt>证据状态</dt><dd>已验证</dd></div></dl></div></div>}
